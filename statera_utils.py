...
POS_WORDS = {"good","great","positive","gain","gains","beat","beats","upgrade","upgraded",
 "strong","outperform","growth","surge","record","raise","raised","profit","profits",
 "guidance","buyback","dividend"}
NEG_WORDS = {"bad","poor","negative","miss","missed","downgrade","downgraded","weak",
 "underperform","decline","drop","falls","loss","losses","cut","lawsuit","probe",
 "recall","fraud","layoffs","ban"}
...
def fetch_news_sentiment(tickers: List[str], api_key: str | None = None, pause: float = 0.4) -> Dict[str, float]:
    ...
    for tk in tickers:
        ...
        for a in arts:
            ...
            lo = text.lower()
            pos = sum(1 for w in {"good","great","positive","gain","gains","beat","upgrade","strong","growth","surge","record","raise","profit","guidance","buyback","dividend"} if w in lo)
            neg = sum(1 for w in {"bad","poor","negative","miss","missed","downgrade","weak","underperform","decline","drop","falls","loss","cut","lawsuit","probe","recall","fraud","layoffs","ban"} if w in lo)
