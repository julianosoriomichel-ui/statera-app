...
def fetch_news_sentiment(tickers: List[str], api_key: str | None = None, pause: float = 0.4) -> Dict[str, float]:
    ...
    for tk in tickers:
        ...
        for a in arts:
            ...
            lo = text.lower()
            pos = sum(1 for w in POS_WORDS if w in lo)
            neg = sum(1 for w in NEG_WORDS if w in lo)
...
def score_stocks(metrics: pd.DataFrame, news_sent: Dict[str,float] | None = None,
    ...
    ns = pd.Series(news_sent, dtype=float).reindex(df.index).fillna(0.0) if news_sent is not None else pd.Series(0.0, index=df.index)
    df["s_news"] = (ns + 1) / 2.0
