...
with st.sidebar:
    ...
    st.caption("Add NEWSAPI_KEY to Streamlit Secrets to enable news-driven signals.")
...
if fetch_btn:
    ...
    if prices is None or prices.empty:
        st.error("No data retrieved. Check tickers or try again.")
    else:
        ...
        if selected:
            weights = {}
            st.caption("Enter raw weights (we’ll normalize them to sum to 1)")
            total = 0.0
            for s in selected:
            ...
            port_rets = prices[selected].pct_change().dropna() @ pd.Series(weights)
            ...
            if st.button("Optimize (Max Sharpe)"):
                ...
                st.write("Optimized weights (capped at 20%):", optw)

            if st.button("Backtest & Risk Report"):
...
if build_btn:
    ...
    if st.button("Backtest Suggested Portfolio"):
        ...
        if cum.empty:
            ...
            c4.metric("CVaR 95%", f"{cvar95:.2%}")
            st.subheader("Risk Decomposition")
            st.dataframe(risk_decomposition(prices[list(capped.keys())], capped))

st.header("3) Exports & Next Steps")
st.markdown("""
- To enable **news-driven** signals, add your NewsAPI key in Streamlit Secrets as `NEWSAPI_KEY`.
- For faster & richer data (fundamentals, full universe), upgrade to Polygon/IEX.
- Available upgrades: PDF export, broker trade CSV, rebalancing alerts.
""")
